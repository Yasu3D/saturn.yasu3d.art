# Adding Locales

SATURN is designed with a fully localizable UI.  
Locales can be selected from the [system test menu](/documentation/saturn/index.html#/operation-info/system_test_menu.md).

Any `.toml` file in `./Locales/` will be recognized as a `locale`.  
Locale files should be named after the [locale code of the language](https://gist.github.com/typpo/b2b828a35e683b9bf8db91b5404f1bd1).

```file tree
.
└── StreamingAssets/
    └── Locales/
        └── en-US.toml
```